 ...
 const handleAuthSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    if (res.ok) {
      if (data.needsVerification) {
        // Case: Registration ya Unverified Login -> Show OTP Screen
        setIsVerifying(true);
        if (isLogin) setIsLogin(false);
      } else {
        // Case: Verified User Login -> Seedha Dashboard
  ...
  return (
    <div className="min-h-screen w-full bg-[#020617] flex items-center justify-center p-4 overflow-x-hidden relative">
      <div className="relative w-full max-w-[1050px] min-h-[600px] md:h-[650px] bg-slate-900/40 backdrop-blur-3xl rounded-[30px] md:rounded-[40px] border border-white/5 shadow-2xl overflow-hidden flex flex-col md:flex-row pt-28 pb-12 md:py-0">
        
        <Link href="/" className="absolute top-8 left-8 z-50 flex items-center gap-2">
        ...
        <div className="relative w-full h-full flex items-center">
          ...
          {/* LOGIN FORM */}
          <section className={`w-full md:w-1/2 flex flex-col justify-center px-8 md:px-16 transition-all duration-700 ${!isLogin ? 'opacity-0 translate-x-12 pointer-events-none hidden md:flex' : 'opacity-100 translate-x-0 flex'}`}>
            ...
            <form onSubmit={handleAuthSubmit}>
              ...
              <button disabled={isLoading} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-3 active:scale-95 shadow-xl transition-all disabled:opacity-50">
                {isLoading ? <Loader2 className="animate-spin" size={20}/> : "Login Portal"} <ArrowRight size={18} />
              </button>
            </form>
            <p className="mt-8 text-slate-500 text-[10px] font-black uppercase text-center block md:hidden">
              New here? <button onClick={toggleAuth} className="text-indigo-400 underline">Create Account</button>
            </p>
          </section>

          {/* SIGNUP / OTP FORM */}
          <section className={`w-full md:w-1/2 absolute md:static top-0 left-0 h-full flex flex-col justify-center px-8 md:px-16 transition-all duration-700 ${isLogin ? 'opacity-0 -translate-x-12 pointer-events-none hidden md:flex' : 'opacity-100 translate-x-0 flex'}`}>
            <AnimatePresence mode="wait">
              {!isVerifying ? (
                <motion.div key="signup-form" className="w-full" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                   ...
                   <form onSubmit={handleAuthSubmit}>
                     ...
                     <AuthInput icon={<Lock size={18}/>} name="password" type="password" placeholder="Password" value={formData.password} onChange={handleInputChange} />
                     <button disabled={isLoading} className="w-full bg-indigo-600 text-white font-bold py-4 rounded-2xl shadow-xl hover:bg-indigo-500 transition-all">
                        {isLoading ? <Loader2 className="animate-spin" size={20}/> : "Get OTP Verification"}
                     </button>
                   </form>
                   <p className="mt-8 text-slate-500 text-[10px] font-black uppercase text-center block md:hidden">
                ...
                <motion.div key="otp-form" className="w-full" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                   ...
                   <form onSubmit={handleVerifyOtp}>
                      <input maxLength={6} placeholder="6 - digit code" value={otpInput} onChange={(e) => setOtpInput(e.target.value)} className="w-full bg-[#0f172a] border border-slate-800 rounded-2xl py-5 text-indigo-400 text-center text-xl font-black tracking-widest outline-none mb-6" />
                      <button disabled={isLoading} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-4 rounded-2xl">
                        {isLoading ? <Loader2 className="animate-spin mx-auto" size={20}/> : "Confirm Account"}
                      </button>
          ...
          {/* DESKTOP OVERLAY */}
          <motion.div animate={{ x: isLogin ? "0%" : "-100%" }} className="absolute top-0 right-0 w-1/2 h-full hidden md:block z-40 overflow-hidden shadow-2xl">
            <div className="relative w-full h-full bg-indigo-600 flex items-center px-14">
              <div className="relative z-10 w-full text-center text-white">
                 ...
                 <button onClick={toggleAuth} className="bg-white text-indigo-800 font-bold py-4 px-12 rounded-2xl uppercase text-[10px] hover:scale-105 transition-all">
